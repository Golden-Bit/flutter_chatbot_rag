// lib/apps/enac_app/ui_components/clients/client_form_page.dart
import 'dart:async';
...
import 'client_form_widget.dart';

class CreateClientPage extends StatefulWidget with ChatBotExtensions {
  ...
  final FutureOr<void> Function(String entityId) onCreated;

  final String title;

  // ── ChatBotExtensions: esponi i tool del ClientFormPane ──────────────
  @override
...
class _CreateClientPageState extends State<CreateClientPage> {
  ...
  Future<void> _onCreatePressed() async {
    ...
    try {
      // genera id client (come nel tuo CreateOrEdit esistente)
      final id = const Uuid().v4().replaceAll('-', '').substring(0, 12);
      await widget.sdk.createEntity(widget.user.username, id, entity);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Entità creata')),
      );
      await widget.onCreated(id);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Errore creazione entità: $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        // ───────── HEADER allineato al bordo destro del form ─────────
        Align(
        ...
        Expanded(
          child: Align(
            ...
            child: ConstrainedBox(
              constraints: const BoxConstraints(maxWidth: _kFormMaxWidth),
              child: ClientFormPane(key: _paneKey),
            ),
          ),
        ),
      ],
