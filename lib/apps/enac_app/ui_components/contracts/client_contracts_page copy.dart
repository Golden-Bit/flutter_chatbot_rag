...
import 'contracts_table.dart';
  ...
  // ⬇️ nuovo: switch pagina contratti / form
  enum _ContractsInnerView { list, form }
  _ContractsInnerView _view = _ContractsInnerView.list;
...
Widget _infoColumn(List<Widget> children) => Column(
  ...
  Widget build(BuildContext context) {
      if (_view == _ContractsInnerView.form) {
    return CreateContractPage(
      user     : widget.user,
      token    : widget.token,
      sdk      : widget.sdk,
      clientId : widget.clientId,
      onCancel : () => setState(() => _view = _ContractsInnerView.list),
      onCreated: (contractId) async {
        setState(() {
          _view = _ContractsInnerView.list;
          _refresh++; // forza il reload della tabella
        });
      },
    );
  }
...
onPressed: () {
  setState(() => _view = _ContractsInnerView.form);
},
