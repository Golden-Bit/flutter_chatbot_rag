...
import 'package:flutter_app/ui_components/chat_message/codeblock_md_builder.dart';
...
class ParsedWidgetResult {
  ...
  ParsedWidgetResult(this.text, this.widgetData);
}
...
ParsedWidgetResult _parsePotentialWidget(String fullText) {
  // 1) Trova l'indice di inizio del pattern "< TYPE='WIDGET'"
  final startIndex = fullText.indexOf("< TYPE='WIDGET'");
  if (startIndex == -1) {
    // Se il pattern non è presente, restituisci il testo intero e widgetData null
    return ParsedWidgetResult(fullText, null);
  }

  // 2) Trova l'indice di chiusura ">" del tag
  final endIndex = fullText.indexOf(">", startIndex);
  if (endIndex == -1) {
    // Se manca proprio il carattere '>', non abbiamo un blocco widget valido
    return ParsedWidgetResult(fullText, null);
  }

  // Estrarre il sottoblocco con tag e contenuto
  final widgetBlock = fullText.substring(startIndex, endIndex + 1);

  // 3) Cerchiamo la prima e la seconda barra verticale "|"
  //    che delimitano il JSON all’interno del blocco
  final firstBar = widgetBlock.indexOf("|");
  final secondBar = widgetBlock.indexOf("|", firstBar + 1);

  if (firstBar == -1 || secondBar == -1) {
    // Se non troviamo due barre verticali, il pattern non è valido
    return ParsedWidgetResult(fullText, null);
  }

  // 4) Estrarre la parte JSON compresa tra le due barre
  final jsonString = widgetBlock.substring(firstBar + 1, secondBar).trim();

  // 5) Estrazione del widgetId
  final widgetIdSearch = "WIDGET_ID='";
  final widgetIdStart = widgetBlock.indexOf(widgetIdSearch);
  if (widgetIdStart == -1) {
    // Se non troviamo proprio la sottostringa WIDGET_ID='
    return ParsedWidgetResult(fullText, null);
  }
  ...
  if (widgetIdEnd == -1 || widgetIdEnd <= widgetIdStartAdjusted) {
    // Se non troviamo la chiusura dell'apice o l'indice è incoerente
    return ParsedWidgetResult(fullText, null);
  }

  final widgetId = widgetBlock.substring(widgetIdStartAdjusted, widgetIdEnd);

  // 6) Prova a decodificare la parte JSON
  ...
  // 7) Se il JSON non è decodificabile, restituiamo solo il testo
  if (widgetJson == null) {
    return ParsedWidgetResult(fullText, null);
  }

  // 8) Rimuovi (o sostituisci) dal fullText la porzione widget con un placeholder
  final cleanedText = fullText.replaceRange(
    startIndex,
    endIndex + 1,
    "[WIDGET_PLACEHOLDER]",
  );

  // *** Nuovo: genera un _id univoco per il widget
  final widgetUniqueId = uuid.v4(); 
  // Nota: Assicurati di aver dichiarato "final Uuid uuid = Uuid();" nella classe

  // 9) Ritorna i dati con anche il "_id"
  return ParsedWidgetResult(
    cleanedText,
    {
      "_id": widgetUniqueId,
      "widgetId": widgetId,
...
Widget _buildMixedContent(Map<String, dynamic> message) {
  // Controlla se esiste un widgetData
  final widgetData = message['widgetData'];

  // Se non c’è widget, possiamo tornare semplicemente
  // un widget Markdown (o quello che preferisci).
  if (widgetData == null) {
    // Non c'è widget => tutto testo
  ...
  // Se invece c’è widgetData, significa che c’è (almeno) un segnaposto nel testo
  final textContent = message['content'] ?? '';
  // Dividiamo il testo in segmenti intorno a "[WIDGET_PLACEHOLDER]"
  // (può funzionare anche se ci sono più placeholder, ma occorre gestire la lista).
  // Per semplicità, supponiamo ci sia un solo placeholder.
  final parts = textContent.split('[WIDGET_PLACEHOLDER]');

  // Preleviamo l'ID del widget e i dati JSON
  final widgetId = widgetData['widgetId'] as String;
  final jsonData = widgetData['jsonData'] as Map<String, dynamic>? ?? {};

  // Per fare caching, usiamo l'ID univoco del messaggio stesso
  // (message['id'] è già generato altrove)
final widgetUniqueId = widgetData['_id'] as String;
Widget? embeddedWidget = _widgetCache[widgetUniqueId];
  if (embeddedWidget == null) {
    // Se non è presente, costruiamo il widget ora
    print(widgetData);
    print(widgetId);
    print(widgetMap);
    ...
    if (widgetBuilder != null) {
      ...
      // Se non troviamo widgetId
      embeddedWidget = Text("Widget sconosciuto: $widgetId");
    }

    // Salviamo l’istanza nella cache per riutilizzarla
    _widgetCache[widgetUniqueId] = embeddedWidget;
  }

  // A questo punto vogliamo rendere i segmenti di testo + (se esiste) il widget in mezzo.
  // Se in `parts` ci sono N blocchi di testo, li mostriamo in successione,
  // e in mezzo ci mettiamo `embeddedWidget` dove c’era il placeholder.
  // Per 1 solo placeholder, avremo 2 parti: [prima del placeholder, dopo il placeholder].
  // Example: parts[0], embeddedWidget, parts[1]

  // Creiamo una lista di Widget finali
  final contentWidgets = <Widget>[];

  for (int i = 0; i < parts.length; i++) {
    final segment = parts[i];
    if (segment.isNotEmpty) {
      // Mostra il testo come Markdown (o come preferisci)
      final isUser = (message['role'] == 'user');
      contentWidgets.add(
    ...
    // Se non siamo all’ultima parte, significa che c’era un placeholder dopo di essa;
    // quindi aggiungiamo il widget, centrato orizzontalmente
    if (i < parts.length - 1) {
      contentWidgets.add(
        Container(
          width: double.infinity, // Rende il riquadro largo quanto la colonna
          child: Align(
            alignment: Alignment.center, // Centra il widget all’interno di quel riquadro
            child: embeddedWidget,       // Il widget preso (o creato) dalla cache
          ),
        ),
      );
    }
  ...
  // Se per qualche ragione non trovavamo "[WIDGET_PLACEHOLDER]", avremmo 1 sola part e nessun widget in mezzo;
  // in tal caso, stiamo mostrando tutto come testo, e poi non aggiungiamo widget (oppure potresti aggiungerlo in coda).

  // Avvolgiamo il tutto in un Column
  return Column(
      ...
      void readChunk() {
        js_util
          ...
          if (!done) {
            ...
            // Nuova parte: parse del widget
            final parsed = _parsePotentialWidget(fullResponse);

// Sovrascrivi il contenuto e aggiungi (eventualmente) widgetData
            setState(() {
              messages[messages.length - 1]['content'] = parsed.text;
              messages[messages.length - 1]['widgetData'] = parsed.widgetData;
            });
