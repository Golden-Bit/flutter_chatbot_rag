...
// ────────────────────────────────────────────────────────────────
// NEW ▸ Global billing cache (riutilizzabile da altre schermate)
// ────────────────────────────────────────────────────────────────
class BillingGlobals {
  static CurrentPlanResponse? currentPlan;
  static UserCreditsResponse? userCredits;
...
// ────────────────────────────────────────────────────────────────
// NEW ▸ kick-off non bloccante delle chiamate payments
// ────────────────────────────────────────────────────────────────
void _kickoffPaymentsFetch() {
  if (_paymentsFetchInFlight) return;
  _paymentsFetchInFlight = true;

  // Avvia senza bloccare l’init. Con Dart moderno puoi usare unawaited;
  // in alternativa chiama semplicemente _fetchPaymentsInfo(); senza await.
...
Future<void> _fetchPaymentsInfo() async {
  try {
    ...
    // 1) Piano corrente
    final plan = await _apiSdk.getCurrentPlan(tok);
    _currentPlan = plan;
    BillingGlobals.currentPlan = plan;

    // 2) Crediti (preferisci passare subscriptionId, ma puoi anche ometterlo)
    UserCreditsResponse credits;
    try {
      credits = await _apiSdk.getUserCredits(tok, subscriptionId: plan.subscriptionId);
    } catch (_) {
      // fallback se il server non accetta sub_id o non presente
      credits = await _apiSdk.getUserCredits(tok);
    }

    _currentCredits = credits;
    BillingGlobals.userCredits = credits;
    BillingGlobals.lastUpdated = DateTime.now();

    // Se qualche widget vuole reagire subito:
    if (mounted) setState(() {});
  } catch (e) {
