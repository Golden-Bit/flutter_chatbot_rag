...
class _ExpandableCodeBlockWidgetState extends State<ExpandableCodeBlockWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    // Verifica se il blocco riguarda HTML.
    ...
    // Se il codice è HTML, aggiunge l'icona di visualizzazione.
    if (isHtml) {
      headerIcons.add(
        IconButton(
          icon: Icon(
    ...
    // Icona di espansione/compressione.
    headerIcons.add(
      IconButton(
        icon: Icon(
    ...
    return Column(
      ...
      children: [
        // Header: mostra il nome del linguaggio ed il gruppo di icone.
        Container(
        ...
        // Corpo del codeblock: il contenuto evidenziato.
        ConstrainedBox(
        ...
        if (isHtml && _showHtmlPreview)
          Container(
            ...
            decoration: BoxDecoration(
              color: Colors.white, // Background per l'anteprima HTML
              border: Border.all(color: Colors.grey.shade300),
              borderRadius: BorderRadius.circular(4.0),
            ),
            child: ConstrainedBox(
              constraints: const BoxConstraints(
                maxHeight: 300, // Altezza massima per l'anteprima HTML
              ),
              child: Scrollbar(
                child: SingleChildScrollView(
                  child: HtmlWidget(
                    widget.code,
                    // customStylesBuilder per personalizzare gli stili (es. per le tabelle)
                    ...
                    // Se necessario, qui si può aggiungere un customWidgetBuilder per elementi specifici.
                  ),
                ),
              ),
            ),
...
/// Utilizza [ExpandableCodeBlockWidget] per gestire l'espansione.
class CodeBlockBuilder extends MarkdownElementBuilder {
  ...
  Widget visitElementAfter(md.Element element, TextStyle? preferredStyle) {
    // Estrae il contenuto testuale del blocco di codice.
    final String code = element.textContent;

    // Determina il linguaggio dal parametro "class", ad esempio "language-dart".
...
/// Questa versione non gestisce blocchi HTML separati tramite marker,
/// poiché il rendering HTML nei code block è gestito internamente (per il linguaggio "html").
Widget buildAdvancedMarkdownMessage(
  ...
  return Container(
    ...
    child: MarkdownBody(
      data: content,
      // Inietta il builder personalizzato per i blocchi di codice.
      builders: {
      ...
      styleSheet: MarkdownStyleSheet(
        ...
        // Lo stile base per il codice (non usato direttamente, poiché il nostro builder gestisce il rendering).
        code: TextStyle(
          fontFamily: 'Courier',
