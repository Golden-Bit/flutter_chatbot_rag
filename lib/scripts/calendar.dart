...
class CalendarComponent extends StatefulWidget {
  ...
  CalendarComponent({required this.token});  // Accetta il token come parametro
...
void initState() {
  super.initState();
  _loadAppointmentsFromDatabase(); // Carica gli appuntamenti dal database all'inizializzazione
}
  ...
  setState(() {
    for (int i = 0; i < recurrenceCount; i++) {
      ...
      // Debug print
      print("Aggiunta appuntamento: ${appointment.toJson()}");

      // Salvataggio nel database
      _saveAppointmentToDatabase(appointment);
    }
  });
}
...
Future<void> _loadAppointmentsFromDatabase() async {
  try {
    ...
    final username = user.username; // Ricava il nome utente
    final dbName = '${username}-database_0';
    final collectionName = 'appointments';

    final appointmentsData = await databaseService.fetchCollectionData(
      dbName,
      collectionName,
      widget.token,
    );
    ...
    print("Appuntamenti caricati dal database");
  } catch (e) {
    print("Errore durante il caricamento degli appuntamenti: $e");
  }
}

Future<void> _saveAppointmentToDatabase(Appointment appointment) async {
  try {
    final authService = AuthService();
    final databaseService = DatabaseService();
    ...
    print("Username utilizzato: $username");
    print("Database utilizzato: $dbName");
    ...
    print("Appuntamento salvato nel database");
  } catch (e) {
    print("Errore durante il salvataggio dell'appuntamento: $e");
  }
}
...
Widget build(BuildContext context) {
  ...
  return Scaffold(
    appBar: AppBar(
      ...
      leading: IconButton( // Aggiungi questa parte per la freccia indietro
...
class Appointment {
  ...
  factory Appointment.fromJson(Map<String, dynamic> json) {
    return Appointment(
      ...
      recurrence: json['recurrence'] ?? 'Nessuna',
      recurrenceCount: json['recurrenceCount'] ?? 1,
      currentRecurrence: json['currentRecurrence'] ?? 1,
