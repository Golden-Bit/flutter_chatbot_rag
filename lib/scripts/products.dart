...
// Import necessario per il DatabaseService
...
class ProductManagerPage extends StatefulWidget {
  ...
  ProductManagerPage({required this.token}); // Accetta il token come parametro
...
class _ProductManagerPageState extends State<ProductManagerPage> {
  ...
  void initState() {
    super.initState();
    _loadProductsFromDatabase();  // Carica i prodotti dal database all'avvio della pagina
  }
 ...
 Future<void> _loadProductsFromDatabase() async {
  ...
  try {
    ...
    final productsData = await databaseService.fetchCollectionData(
      databaseName,
      'products',
      token,
    );
...
void _addOrUpdateProduct(Product product, {Product? existingProduct}) async {
  final databaseService = DatabaseService();
  final authService = AuthService();
  final token = widget.token;
  ...
  try {
    ...
    if (existingProduct != null) {
      ...
      product.databaseId = existingProduct.databaseId;

      // Aggiorna il prodotto nel database
      await databaseService.updateCollectionData(
        databaseName,
        'products',
        product.databaseId!, // Usa databaseId per aggiornare
        product.toJson(),
        token,
      );
    } else {
      ...
      product.databaseId = response['id'];
    ...
    print("Errore durante l'aggiornamento o l'aggiunta del prodotto: $e");
  }
}
...
void _removeProduct(Product product) async {
  ...
  try {
    await databaseService.deleteCollectionData(
      ...
      'products',
      product.databaseId!, // Usa databaseId per eliminare
      token,
    );
  ...
  // Modifica la funzione _duplicateProduct per salvare il prodotto duplicato nel database
void _duplicateProduct(Product product) {
  final duplicatedProduct = Product(
    ...
    sku: '${product.sku}_copy', // Assuming SKU is unique, so appending '_copy'
  ...
  Navigator.push(
    ...
    MaterialPageRoute(
      builder: (context) => AddProductPage(
        onAddOrUpdateProduct: (newProduct, {existingProduct}) =>
...
class Product {
  ...
  String? databaseId; // Nuovo campo opzionale per memorizzare l'ID nel database
  ...
  Product({
    ...
    this.databaseId, // Inizializza il nuovo campo
  });
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'additionalDescriptions': additionalDescriptions
          .map((desc) => {'title': desc.title, 'content': desc.content})
          .toList(),
      ...
      'parts': parts.map((part) => {
        'product': part.product.toJson(),
        'quantity': part.quantity
      }).toList(),
      'productType': productType,
      'databaseId': databaseId, // Aggiungi il nuovo campo al JSON
    };
  }
  ...
  factory Product.fromJson(Map<String, dynamic> json) {
    return Product(
      ...
      additionalDescriptions: (json['additionalDescriptions'] as List)
          .map((desc) => AdditionalDescription(
                title: desc['title'],
                content: desc['content'],
              ))
          .toList(),
      categories: (json['categories'] as List)
          .map((category) => Label(name: category, color: Colors.grey)) // Default color
          .toList(),
      salePrice: json['salePrice'],
      purchasePrice: json['purchasePrice'],
      billingPolicy: json['billingPolicy'],
      taxes: (json['taxes'] as List)
          .map((tax) => Tax(name: tax['name'], rate: tax['rate']))
          .toList(),
      ...
      labels: (json['labels'] as List)
          .map((label) => Label(name: label, color: Colors.grey)) // Default color
          .toList(),
      ...
      parts: (json['parts'] as List)
          .map((part) => ProductPart(
                Product.fromJson(part['product']),
                part['quantity'],
              ))
          .toList(),
...
class AddProductPage extends StatefulWidget {
  final Function(Product, {Product? existingProduct}) onAddOrUpdateProduct;
...
void _saveProduct({Product? existingProduct}) {
  final product = Product(
    ...
    databaseId: existingProduct?.databaseId, // Mantieni l'ID se esiste
  );

  widget.onAddOrUpdateProduct(product, existingProduct: existingProduct);
  Navigator.of(context).pop();
}
