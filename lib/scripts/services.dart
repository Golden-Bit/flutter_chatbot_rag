...
class ServiceManagerPage extends StatefulWidget {
  final String token;  // Aggiungi il token come parametro richiesto

  ServiceManagerPage({required this.token});
...
void initState() {
  super.initState();
  _loadServicesFromDatabase();  // Carica i servizi dal database all'avvio della pagina
}
...
Future<void> _loadServicesFromDatabase() async {
  ...
  try {
    ...
    final databaseName = "${currentUser.username}-database_0";

    final servicesData = await databaseService.fetchCollectionData(
      databaseName,
      'services',
      token,
    );
  ...
  void _addOrUpdateService(Service service, {Service? existingService}) async {
  final databaseService = DatabaseService();
  final authService = AuthService();
  final token = widget.token;
  ...
  try {
    ...
    if (existingService != null) {
      ...
      await databaseService.updateCollectionData(
        databaseName,
        'services',
        service.databaseId!, // Usa databaseId per aggiornare
        service.toJson(),
        token,
      );
    } else {
      ...
      // Aggiorna il campo databaseId con l'ID ritornato dalla risposta
      service.databaseId = response['id'];
    ...
    print("Errore durante l'aggiornamento o l'aggiunta del servizio: $e");
  }
}


void _removeService(Service service) async {
  final authService = AuthService();
  final databaseService = DatabaseService();
  final token = widget.token;
  ...
  try {
    await databaseService.deleteCollectionData(
      "${(await authService.fetchCurrentUser(token)).username}-database_0",
      'services',
      service.databaseId!, // Usa databaseId per eliminare
      token,
    );
    ...
    print("Errore durante l'eliminazione del servizio: $e");
  }
}
 ...
 void _duplicateService(Service service) {
  final duplicatedService = Service(
    ...
    databaseId: null, // Imposta databaseId a null per il duplicato
  );
  ...
  Navigator.push(
    ...
    MaterialPageRoute(
      builder: (context) => AddServicePage(
        onAddOrUpdateService: (newService, {existingService}) =>
            _addOrUpdateService(newService, existingService: existingService),
...
class Service {
  ...
  String? databaseId; // Aggiunto campo databaseId
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'additionalDescriptions': additionalDescriptions.map((desc) => {'title': desc.title, 'content': desc.content}).toList(),
      'categories': categories.map((category) => category.name).toList(),
      'salePrice': salePrice,
      'purchasePrice': purchasePrice,
      'billingPolicy': billingPolicy,
      'taxes': taxes.map((tax) => {'name': tax.name, 'rate': tax.rate}).toList(),
      ...
      'parts': parts.map((part) => {'service': part.service.toJson(), 'quantity': part.quantity}).toList(),
      'serviceType': serviceType,
      'databaseId': databaseId, // Aggiunto al JSON
    };
  }
  ...
  factory Service.fromJson(Map<String, dynamic> json) {
    return Service(
      ...
      additionalDescriptions: (json['additionalDescriptions'] as List).map((desc) => AdditionalDescription(title: desc['title'], content: desc['content'])).toList(),
      categories: (json['categories'] as List).map((category) => Label(name: category, color: Colors.grey)).toList(),
      salePrice: json['salePrice'],
      purchasePrice: json['purchasePrice'],
      billingPolicy: json['billingPolicy'],
      taxes: (json['taxes'] as List).map((tax) => Tax(name: tax['name'], rate: tax['rate'])).toList(),
      servicecode: json['servicecode'],
      attachments: List<String>.from(json['attachments']),
      labels: (json['labels'] as List).map((label) => Label(name: label, color: Colors.grey)).toList(),
      images: List<String>.from(json['images']),
      currency: json['currency'],
      unitOfMeasure: json['unitOfMeasure'],
      minQuantity: json['minQuantity'],
      minIncrement: json['minIncrement'],
      parts: (json['parts'] as List).map((part) => ServicePart(Service.fromJson(part['service']), part['quantity'])).toList(),
      serviceType: json['serviceType'],
      databaseId: json['_id'] ?? json['id'], // Recupera l'ID dal database
    );
  }
}
...
class AddServicePage extends StatefulWidget {
  final Function(Service, {Service? existingService}) onAddOrUpdateService;
...
void initState() {
  ...
  final service = Service(
    ...
    description: descriptionController.text.isNotEmpty ? descriptionController.text : 'Non compilato',
    ...
    taxes: addedTaxes.isNotEmpty ? addedTaxes : [Tax(name: 'Non compilato', rate: 0.0)],
    servicecode: servicecodeController.text.isNotEmpty ? servicecodeController.text : 'Non compilato',
    attachments: attachments,
    images: images,
    currency: currency,
    unitOfMeasure: selectedUnitOfMeasure.isNotEmpty ? selectedUnitOfMeasure : 'Non compilato',
    ...
    databaseId: widget.existingService?.databaseId, // Mantieni l'ID se esiste
  );

  widget.onAddOrUpdateService(service, existingService: widget.existingService);
  Navigator.of(context).pop();
}
